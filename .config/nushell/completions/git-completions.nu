...
def "nu-complete git remotes" [] {
  let out = (do { ^git remote --verbose } | complete)
...
def "nu-complete git files" [path?: string] {
  ...
  let target = (if ($path | describe) == 'string' and ($path | str length) != 0 { $"($path)" } else { "." })
  let status_output = (do { ^git status -uall --porcelain=2 $target } | complete)
...
def "nu-complete worktree list" [] {
  let out = (do { ^git worktree list } | complete)
  if $out.exit_code != 0 or ($out.stdout | is-empty) { return [] }
