...
Item {
  ...
  // sender. The helper matches the pattern against the window class,
  // case-insensitively, but notification app names do not always look like
  // window classes: Telegram Desktop notifies as "Telegram Desktop" while its
  // class is org.telegram.desktop, and Chromium browsers notify as
  // "Google Chrome" while their class is google-chrome. Joining the words of
  // the app name with ".*" lets the tokens match across punctuation.
  function focusApp(entry) {
    if (!entry || !entry.app) return
    focusAppProc.command = [
      service.omarchyPath + "/bin/omarchy-hyprland-focus-app",
      focusAppPattern(entry.app)
    ]
    focusAppProc.running = true
  }

  function focusAppPattern(app) {
    var words = String(app).split(/[^A-Za-z0-9]+/).filter(function(word) { return word.length > 0 })
    return words.length > 0 ? words.join(".*") : String(app)
  }
