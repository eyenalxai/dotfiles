...
Panel {
  ...
  function batteryIcon() {
    var device = UPower.displayDevice
    return Model.batteryIcon(device, UPower.onBattery, upowerStates(), root.batteryInfo)
  }

  function modeLabel() {
    var device = UPower.displayDevice
    return Model.modeLabel(device, UPower.onBattery, upowerStates(), root.batteryInfo)
  }
  ...
  readonly property bool discharging: {
    var device = UPower.displayDevice
    return Model.isDeviceDischarging(device, UPower.onBattery, upowerStates(), root.batteryInfo)
  }
  readonly property bool charging: {
    var device = UPower.displayDevice
    return Model.isDeviceCharging(device, UPower.onBattery, upowerStates(), root.batteryInfo)
  }
  readonly property bool chargeThresholdActive: {
    var device = UPower.displayDevice
    return Model.chargeThresholdActive(device, UPower.onBattery, upowerStates(), root.batteryInfo)
  }
  readonly property bool fullyCharged: {
    var device = UPower.displayDevice
    return device && device.isPresent && (device.state === UPowerDeviceState.FullyCharged || batteryFraction >= 0.99) && !root.chargeThresholdActive && !root.discharging
  }
  ...
  readonly property string batteryScript: {
    var resolved = Qt.resolvedUrl("battery-status.sh").toString().replace(/^file:\/\//, "")
          ...
          Column {
            ...
            InfoPair {
              label: root.chargeThresholdActive
                ? "Charge limit"
                : (root.discharging || root.batteryInfo.time_type === "empty" ? "Time left" : "Time to full")
