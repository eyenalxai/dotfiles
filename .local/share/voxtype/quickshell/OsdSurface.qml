...
PanelWindow {
    ...
    /// Check if this panel is on the currently focused monitor
    readonly property bool isFocusedMonitor: {
        if (!panel.screen || !Hyprland.focusedMonitor) return true;
        return panel.screen.name === Hyprland.focusedMonitor.name;
    }

    visible: !osdSuppressed && daemonState !== "idle" && daemonState !== "" && panel.isFocusedMonitor
    anchors {
        top: true
        bottom: true
        left: true
